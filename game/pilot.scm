...
(define hilou (mecs-new-var))

(define show-home
  (mecs-new-func (lambda (home)    ; -> ()
                   (skip-if (not home))
                   (print "わたしの家")
                   (print "お母さん「鍛冶屋さんで包丁を作ってもらって、」")
                   (print "お母さん「ヒロウ村のおじいちゃんに持って行ってちょうだい」")
                   (values))))
(mecs-connect! show-home `(,home) ())
...
(define show-smith
  (mecs-new-func (lambda (smith has-stone?)    ; -> ()
                   (skip-if (not smith))
                   (print "鍛冶屋")
                   (if has-stone?
                       (begin
                   ...
                   (values))))
(mecs-connect! show-smith `(,smith ,has-stone?) ())
...
(mecs-connect! home->smith `(,ui-home->smith) `(,home ,smith))
(mecs-connect! smith->home `(,ui-smith->home) `(,home ,smith))
...
(define hill->smith
 (mecs-new-func (lambda (ui-hill->smith) ; -> smith hill
                  ...
                  (print "鍛冶屋へ移動")
                  (values #t #f))))
(mecs-connect! smith->hill `(,ui-smith->hill) `(,smith ,hill))
(mecs-connect! hill->smith `(,ui-hill->smith) `(,smith ,hill))
...
(define smith->hilou
 (mecs-new-func (lambda (ui-smith->hilou) ; -> smith hilou
                  ...
                  (print "ヒロウ村へ移動")
                  (values #f #t))))
(mecs-connect! smith->hilou `(,ui-smith->hilou) `(,smith ,hilou))

(define show-hilou
  (mecs-new-func (lambda (hilou)    ; -> ()
                   (skip-if (not hilou))
                   (print "ヒロウ村")
                   (print "わたし「おじちゃんはどこかな？」")
                   (print "村人「おじいちゃんは港の近くの家にいるよ」")
                   (values))))
(mecs-connect! show-hilou `(,hilou) ())
