...
(define (desc text . procs)
  (for-each (^(proc) (proc)) procs))
...
(desc "Exception"
      (lambda ()
        (let ((n1 (mecs-new-var))
              (n2 (mecs-new-var))
              (n3 (mecs-new-var))
              (f1 (mecs-new-func (lambda (a b)
                                   (if (> a b)
                                       (- a b)
                                       (raise (condition (<mecs-skip-calculation>))))))))
          (mecs-connect! f1 `(,n1 ,n2) `(n3))
          (mecs-update! `((,n1 . 10) (,n2 . 20)))
          (test* "not modified" #f (mecs-var-defined? (mecs-var-node-var n3)))
          )
        )
      (lambda ()
        (let ((n1 (mecs-new-var))
          ...
          (mecs-connect! f1 `(,n1 ,n2) `(n3))
          (test* "shouldn't handle other exception types" (test-error <condition>)
                 (mecs-update! `((,n1 . 10) (,n2 . 20))))
          (test* "not modified" #f (mecs-var-defined? (mecs-var-node-var n3))))
        ))
